          ...
          >
            <div class="card-body">
              ...
              <h6 class="card-subtitle mb-2 text-muted">
                Игра:
                <span class="price">{{model.game_id}}</span>
              </h6>
              <h6 class="card-subtitle mb-2 text-muted">
                Комиссия сервиса:
                <span class="price">{{model.commission}}</span>
              </h6>
              <div class="row">
              ...
              <div class="row">
                <div class="col">
                  ...
                  <b-form-checkbox
                    ...
                    switch
                  >Показывать цену</b-form-checkbox>
                  <b-form-checkbox
                    v-model="showCount"
                    v-bind:true-value="1"
                    v-bind:false-value="0"
                    switch
                  >Показывать количество</b-form-checkbox>
                  <b-form-checkbox
                    v-model="showTime"
                    v-bind:true-value="1"
                    v-bind:false-value="0"
                    switch
                  >Показывать время</b-form-checkbox>
                  <b-form-checkbox
                    v-model="multipleSale"
                    v-bind:true-value="1"
                    v-bind:false-value="0"
                    switch
                  >Можно продавать несколько раз</b-form-checkbox>
                  <b-form-checkbox
                    v-model="status"
                    v-bind:true-value="1"
                    v-bind:false-value="0"
                    switch
                  >
                    <p v-if="model.status == 1">Видна всем</p>
                    <p v-if="model.status == 0">Не видна</p>
                  </b-form-checkbox>
                  <b-form-checkbox
                    v-model="instantDelivery"
                    v-bind:true-value="1"
                    v-bind:false-value="0"
                    switch
                  >Моментальная доставка</b-form-checkbox>
                  <b-form-checkbox
                    v-model="allowMultipleOffers"
                    v-bind:true-value="1"
                    v-bind:false-value="0"
                    switch
                  >Можно создать несколько предложений в этой категории</b-form-checkbox>
                </div>
              </div>
              <div class="row">
    ...
    exchangeData(rootCom, from, to) {
      ...
      if (this.hasInclude(from, to)) {
        ...
        if (tempParent.$options._componentTag === 'Categories') {
          ...
          this.$store.dispatch({
            type: 'categories/setNewChildren',
            category: toModel,
            newChildren
          })
          return
        ...
        this.$store.dispatch({
          type: 'categories/concatChildren',
          category: toParentModel,
          newChildren
        })
        return
      }
      ...
      if (from.$parent.$options._componentTag === 'Categories') {
        ...
        this.$store.dispatch({
          type: 'categories/setNewChildren',
          category: fromParentModel,
          newChildren: fromParentModel.children.filter(
            (item) => item.id !== newFrom.id
          )
        })
      }
